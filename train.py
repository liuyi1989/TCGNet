...
def main():
    ...
    net = TCGNet(backbone_path).cuda().train()
...
def train(net, optimizer):
    ...
    for epoch in range(args['last_epoch'] + 1, args['last_epoch'] + 1 + args['epoch_num']):
        model_dir = './ckpt/PFNet/' + str(epoch) + '.pth'
        loss_record, loss_3_record = AvgMeter(), AvgMeter()
        ...
        for data in train_iterator:
            ...
            predict_3 = net(inputs)
            # print(cm)
            # predict_1 = torch.sigmoid(predict_1)
            # predict_2 = torch.sigmoid(predict_2)

            # loss_1 = F.binary_cross_entropy(predict_1, labels)
            # loss_2 = F.binary_cross_entropy(predict_2, labels)
